...
- For each failing test, run `test_debug` to pause execution at failure points
...
- Check if selectors match current UI (use `browser_generate_locator` if needed)
...
- Restart the test after each fix using `test_run` or `test_debug`
...
- Use `browser_snapshot` to see current page state when test fails
- Helps identify if UI matches expected state
- Shows what elements are actually present

### Console Messages

- Check `browser_console_messages` for ModelExecutor logs
...
- Use `browser_network_requests` for navigation-related failures
