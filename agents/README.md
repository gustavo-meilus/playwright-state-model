...
Planner agent explores your app and produces a test plan for one or many scenarios and user flows using model-based testing principles. All test plans include parallelism safety requirements and race condition prevention guidelines.
...
- Generates parallelism-safe tests with complete test isolation
- Ensures each test creates its own executor and factory instances
- Prevents race conditions by avoiding shared state
- Verifies tests pass with `--repeat-each 10 --workers 5`
...
- Verifies parallelism safety by running tests with `--repeat-each 10 --workers 5`
- Fixes race conditions and shared state issues
...
- **Create fresh executor and factory instances per test** (critical for parallelism safety)
...
- **Maintain complete test isolation** - no shared state between tests
